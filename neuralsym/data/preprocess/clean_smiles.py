...
def parse_args():
    ...
    # file paths
    parser.add_argument(
        "--raw_smi_pre", help="File prefix of original raw rxn_smi csv", type=str, default="raw")
    parser.add_argument("--clean_smi_pre", help="File prefix of cleaned rxn_smi pickle",
                        type=str, default="50k_clean_rxnsmi_noreagent_allmapped")
    parser.add_argument(
        "--raw_smi_root", help="Full path to folder containing raw rxn_smi csv", type=str)
    parser.add_argument(
        "--clean_smi_root", help="Full path to folder that will contain cleaned rxn_smi pickle", type=str)

    # args for clean_rxn_smis_50k_all_phases
    parser.add_argument(
        "--split_mode", help='Whether to keep rxn_smi with multiple products: "single" or "multi"', type=str, default="multi")
    parser.add_argument("--lines_to_skip",
                        help="Number of lines to skip", type=int, default=1)
    parser.add_argument(
        "--keep_reag", help="Whether to keep reagents in output SMILES string", type=bool, default=False)
    parser.add_argument(
        "--keep_all_rcts", help="Whether to keep all rcts even if they don't contribute atoms to product", type=bool, default=False)
    parser.add_argument(
        "--remove_dup_rxns", help="Whether to remove duplicate rxn_smi", type=bool, default=True)
    parser.add_argument("--remove_rct_mapping",
                        help="Whether to remove atom map if atom in rct is not in product", type=bool, default=True)
    parser.add_argument("--remove_all_mapping",
                        help="Whether to remove all atom map", type=bool, default=False)
    parser.add_argument(
        "--save_idxs", help="Whether to save all bad indices to a file in same dir as clean_smi", type=bool, default=False)
    parser.add_argument(
        "--parallelize", help="Whether to parallelize computation across all available cpus", type=bool, default=True)
...
def clean_rxn_smis_50k_all_phases(
    ...
    **kwargs
    ...
    if input_root is None:
        input_root = Path(__file__).resolve(
        ).parents[2] / "data" / "original_data"
    if output_root is None:
        output_root = Path(__file__).resolve(
        ).parents[2] / "data" / "cleaned_data"
    ...
    for phase in phase_to_compute:
        ...
        if save_idxs:
            ...
            with open(
                output_root /
                    f"{output_file_prefix}_{phase}_badidxs.pickle", "wb"
            ) as handle:
...
def remove_overlapping_rxn_smis(
    ...
    for phase in rxn_smi_phases:
        with open(root / f'{rxn_smi_file_prefix}_{phase}.pickle', 'rb') as handle:
            ...
            rxn_smi_phases_set[phase] = set(rxn_smi_phases[phase])

    overlap_idxs = {'test_in_train': [],
                    'test_in_valid': [], 'valid_in_train': []}
