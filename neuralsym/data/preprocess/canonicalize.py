...
def canonicalize_rxn_smi(
    ...
    if remove_mapping:
        ...
        # double canonicalize
        prod_smi_nomap = Chem.MolToSmiles(
            Chem.MolFromSmiles(prod_smi_nomap), True)
...
def canonicalize_phases(
    ...
    for phase in phases:
        ...
        if input_data_folder is None:
            input_data_folder = Path(__file__).resolve(
        ...
        with open(input_data_folder / f'{input_data_file_prefix}_canon_{phase}.pickle', 'wb') as handle:
            # 4 compatible w/ py36
            pickle.dump(rxn_smis_nodup, handle, protocol=4)
...
if __name__ == '__main__':
    ...
    for prefix, remove in zip(input_prefixes, remove_mapping):
        canonicalize_phases(input_data_file_prefix=prefix,
                            remove_mapping=remove)
        remove_overlapping_rxn_smis(rxn_smi_file_prefix=f'{prefix}_canon')
