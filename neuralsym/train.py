...
def train(args):
    ...
    logging.info(
        f'Total number of template patterns: {len(templates_filtered)}')
    ...
    valid_loader = DataLoader(
        valid_dataset, batch_size=args.bs_eval, shuffle=False)
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:
            ...
            outputs = torch.where(
                (labels.view(-1, 1).expand_as(outputs) < len(templates_filtered)
                 ), outputs, torch.tensor([0.], device=outputs.device)
            )
            labels = torch.where(
                (labels < len(templates_filtered)), labels, torch.tensor(
                    [0.], device=labels.device).long()
            )
            ...
            for k in k_to_calc:
                ...
                train_correct[k] += torch.where(
                    batch_preds == labels.view(-1, 1).expand_as(batch_preds))[0].shape[0]
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(valid_loader):
                ...
                for k in k_to_calc:
                    batch_preds = torch.topk(outputs, k=k, dim=1)[1]
                    valid_correct[k] += torch.where(
                        batch_preds == labels.view(-1, 1).expand_as(batch_preds))[0].shape[0]

                valid_loader.set_description(
                    f"validating: top-1 acc={valid_correct[1]/valid_seen:.4f}")
                ...
                try:
                    for j in range(i * args.bs_eval, (i+1) * args.bs_eval):
                        if j % (valid_size // 5) == random.randint(0, 3) or j % (valid_size // 8) == random.randint(0, 4):
                            batch_preds = torch.topk(outputs, k=1)[
                                1].squeeze(dim=-1)

                            rxn_idx = random.sample(
                                list(range(args.bs_eval)), k=1)[0]
                            rxn_true_class = labels[rxn_idx]
                            rxn_pred_class = int(batch_preds[rxn_idx].item())
                            rxn_pred_score = outputs[rxn_idx,
                                                     rxn_pred_class].item()
                            rxn_true_score = outputs[rxn_idx,
                                                     rxn_true_class].item()

                            # load template database
                            rxn_pred_temp = templates_filtered[rxn_pred_class]
                            rxn_true_temp_idx = int(
                                proposals_data_valid.iloc[idxs[rxn_idx].item(), 4])
                            ...
                            rxn_true_prod = proposals_data_valid.iloc[idxs[rxn_idx].item(
                            ), 1]
                            rxn_true_prec = proposals_data_valid.iloc[idxs[rxn_idx].item(
                            ), 2]
                            ...
                            logging.info(
                                f'pred template:                          \t{rxn_pred_temp}')
                            logging.info(
                                f'true template:                          \t{rxn_true_temp}')
                            if rxn_pred_class == len(templates_filtered):
                                logging.info(
                                    f'pred precursor (score = {rxn_pred_score:+.4f}):\t\tNULL template')
                            elif len(rxn_pred_prec) == 0:
                                logging.info(
                                    f'pred precursor (score = {rxn_pred_score:+.4f}):\t\tTemplate could not be applied')
                            else:
                                logging.info(
                                    f'pred precursor (score = {rxn_pred_score:+.4f}):\t\t{rxn_pred_prec}')
                            logging.info(
                                f'true precursor (score = {rxn_true_score:+.4f}):\t\t{rxn_true_prec}')
                            break
    ...
    logging.info(
        f'Finished training, total time (minutes): {(time.time() - start) / 60}')
    return model
...
def test(model, args):
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=args.bs_eval, shuffle=False)
    del test_dataset
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            for k in k_to_calc:
                batch_preds = torch.topk(outputs, k=k, dim=1)[1]
                test_correct[k] += torch.where(
                    batch_preds == labels.view(-1, 1).expand_as(batch_preds))[0].shape[0]

            test_loader.set_description(
                f"testing: top-1 acc={test_correct[1]/test_seen:.4f}")
            test_loader.refresh()
            ...
            try:
                for j in range(i * args.bs_eval, (i+1) * args.bs_eval):
                    if j % (test_size // 5) == random.randint(0, 3) or j % (test_size // 8) == random.randint(0, 4):
                        ...
                        rxn_pred_temp = templates_filtered[rxn_pred_class]
                        rxn_true_temp_idx = int(
                            proposals_data_test.iloc[idxs[rxn_idx].item(), 4])
                        if rxn_true_temp_idx < len(templates_filtered) and rxn_true_class < len(templates_filtered):
                            ...
                            rxn_true_temp = 'Template not in training data'
                            rxn_true_score = 'N/A'
                        rxn_true_prod = proposals_data_test.iloc[idxs[rxn_idx].item(
                        ), 1]
                        rxn_true_prec = proposals_data_test.iloc[idxs[rxn_idx].item(
                        ), 2]
...
def parse_args():
    ...
    parser.add_argument("--name", type=str, default="Highway")
    parser.add_argument("--do_train", action="store_true")
    parser.add_argument("--do_test", action="store_true")
    parser.add_argument("--model", type=str, default='Highway')
    # file names
    parser.add_argument("--log_file", type=str, default="train")
    parser.add_argument("--templates_file", type=str,
                        default="50k_training_templates")
    parser.add_argument("--prodfps_prefix", type=str)
    parser.add_argument("--labels_prefix", type=str)
    parser.add_argument("--csv_prefix", type=str)
    parser.add_argument("--radius", type=int, default=2)
    parser.add_argument("--min_freq", type=int, default=1)
    parser.add_argument("--fp_size", type=int, default=32681)
...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        checkpoint = torch.load(
            CHECKPOINT_FOLDER / f"{args.name}.pth.tar",
            map_location=device,
        )
        if args.model == 'Highway':
